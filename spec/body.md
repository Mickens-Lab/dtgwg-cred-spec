...
- carries a `threadId` equal to the credential's `taskContext` (the initiating document's `id`, which the exchange's thread carries by the core specification's §4.9 fallback);
