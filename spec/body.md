...
```json
{
  ...
  "taskDigestMultibase": "zQmTCMqL3oJHwM4FQHvjnPBEfXjcGAEN39ZvkDNzSo34qwa",
  "credentialSubject": {
    "id": "did:key:z6MkpTHR8VNs...",
...
- Uniqueness comes from the core specification rather than from issuer discipline: a document's [`id`](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/fbe196a8a17ba3f99d0657a64be5ac58621023a1/SPEC.md#43-the-id-member) is "globally unique to this instance of the task" and producers "MUST NOT reuse an `id` value across documents" (framework 0.4, §4.3) — a normative obligation on a mandatory member. The exchange's `threadId` coincides with this value by the core specification's own fallback (§4.9), so thread-correlated evidence still matches, but the anchor is the `id`: [`threadId`](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/fbe196a8a17ba3f99d0657a64be5ac58621023a1/SPEC.md#49-the-threadid-member) remains optional in the core specification and carries no normative validation semantics.

**The `id` locates; it does not bind.** §4.3's uniqueness obligation constrains conforming producers — it constrains nobody else. An adversary can mint a different document carrying the same `id`, and a verifier pairing the credential with that counterfeit by `id` equality alone would accept evidence of the wrong event. A credential type that marks `taskContext` REQUIRED therefore also marks **`taskDigestMultibase`** REQUIRED: a digest of the initiating document, computed over its JSON representation excluding its `proof` member, canonicalized with JCS (RFC 8785), in the same multihash/multibase encoding as `digestMultibase`. The two properties split one job — `taskContext` is the locator (an O(1) lookup key, conformant with §4.9.1's citation rule), `taskDigestMultibase` is the binder (a counterfeit with the right `id` has the wrong bytes, so its digest cannot match). The digest excludes `proof` so the value is well-defined for unproofed documents and identical across proofed and unproofed forms of the same document.
...
1. declares `proof` as REQUIRED on its success-response variant, using the core specification's per-variant proof-requirement declaration (framework 0.4, [§7.3](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/fbe196a8a17ba3f99d0657a64be5ac58621023a1/SPEC.md#73-specification-requirements)), so the resulting outcome evidence is integrity-protected. (A task specification cannot strengthen the *error* variant: an error response's `type` names the reserved [`trust-task-error`](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/fbe196a8a17ba3f99d0657a64be5ac58621023a1/SPEC.md#8-error-responses) specification, whose declaration no other specification can override — which is one reason error responses are not completion evidence under [Outcome Interpretability](#outcome-interpretability).) And
